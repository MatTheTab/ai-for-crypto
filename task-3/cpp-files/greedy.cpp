...
tuple<vector<uint8_t>, long long, vector<int>> greedy_algorithm(
        ...
        for (int i = 0; i < 256; i++) {
            for (int j = i + 1; j < 256; j++) {
                ...
                if (new_nl > curr_nl) {
                    curr_arr = new_arr;
                    curr_nl = new_nl;
                    curr_whs = whs_metric.get(new_arr);
                    improved = true;
...
int main(int argc, char** argv) {
    ...
    for (int run = 1; run <= num_runs; run++) {
        ...
        auto [best, iterations_used, progress] =
            greedy_algorithm(whs_metric, mode, iter_limit, time_limit, 104);
